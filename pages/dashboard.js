...
Pages.dashboard = function() {
  ...
  return `
    <div class="page-header">
      <h2>Dashboard</h2>
      <p>Welcome back — ${new Date().toLocaleDateString('en-CA', {weekday:'long', year:'numeric', month:'long', day:'numeric'})}</p>
    </div>
    ...
    <div class="alert-card alert-gold">
      <div>
        ...
        <span style="margin-left:12px;color:var(--text-muted);font-size:13px">${quoteRequests.map(q => App.esc(q.customer) + (q.urgency==='emergency'?' 🚨':'')).join(', ')}</span>
      </div>
      ...
      return `
        <div class="alert-card alert-gold">
          <div>
            ...
            <span style="margin-left:12px;color:var(--text-muted);font-size:13px">${dueFollowUps.map(f => App.esc(f.customerName) + ' — ' + App.formatCurrency(f.invoiceTotal)).join(', ')}</span>
          </div>
    ...
    <div class="alert-card alert-gold">
      <div>
        ...
        <span style="margin-left:12px;color:var(--text-muted);font-size:13px">${upcomingMaint.map(p => App.esc(p.customer) + ' — ' + App.formatDate(p.nextVisit)).join(', ')}</span>
      </div>
    ...
    <div class="alert-card alert-gold">
      <div>
        ...
        <span style="margin-left:12px;color:var(--text-muted);font-size:13px">${Warranty.getExpiring().map(w => App.esc(w.customer) + ' — ' + App.formatDate(w.expiryDate)).join(', ')}</span>
      </div>
