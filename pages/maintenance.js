...
PageInit.maintenance = function() {
  ...
  const iconMap = {
    'stat-maint-active': 'maintenance',
    'stat-maint-mrr': 'invoices',
    'stat-maint-arr': 'invoices',
    'stat-maint-billed': 'invoices',
  ...
  viewHistory(id) {
    ...
    App.openModal(`
      <div class="modal-header">
        <h3>Billing History — ${App.esc(p.planName)}</h3>
        <button class="modal-close" onclick="App.closeModal()">✕</button>
      </div>
      <div style="margin-bottom:12px">
        <strong>Customer:</strong> ${App.esc(p.customer) || '(none)'}<br>
        <strong>Plan:</strong> ${App.esc(p.planName)} — ${App.formatCurrency(p.price)}/${p.frequency.toLowerCase()}<br>
  ...
  edit(id) {
    ...
    App.openModal(`
      ...
      <div class="form-group">
        ...
        <input class="form-control" id="mf-cust" value="${App.esc(p.customer)}" list="customer-list" placeholder="Start typing for suggestions...">
        <datalist id="customer-list">${customerOptions}</datalist>
      </div>
      <div class="form-group">
        <label>Plan Name</label>
        <input class="form-control" id="mf-name" value="${App.esc(p.planName)}">
      </div>
      ...
      <div class="form-group">
        ...
        <textarea class="form-control" id="mf-svc" rows="3">${App.esc(p.services||'')}</textarea>
      </div>
      <div class="form-group">
        <label>Next Visit</label>
        <input class="form-control" type="date" id="mf-next" value="${App.esc(p.nextVisit)}">
      </div>
      <div class="form-group">
        <label>Status</label>
