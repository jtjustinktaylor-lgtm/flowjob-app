...
Pages.storage = function() {
  ...
  const categories = [
    ...
    { key: 'warrantyExtensions', label: 'Warranty Extensions', icon: '🛡️' },
    { key: 'warranties', label: 'Warranty Tracker', icon: '🛡️' },
  ];
  ...
  // Backup info
  const backupKeys = Object.keys(localStorage).filter(k => k.startsWith('flowjob_backup_'));
  ...
  const oldBackups = backupKeys.filter(k => {
    const dateStr = k.replace('flowjob_backup_', '');
          ...
          <table class="table" style="margin-top:8px;font-size:12px">
            ...
            <tbody>
              ${backupKeys.sort().reverse().map(k => {
                const date = k.replace('flowjob_backup_', '');
...
const StorageManager = {
  ...
  cleanOldBackups() {
    const keys = Object.keys(localStorage).filter(k => k.startsWith('flowjob_backup_'));
    const weekAgo = new Date();
    weekAgo.setDate(weekAgo.getDate() - 7);
    let removed = 0;
    keys.forEach(k => {
  ...
  archiveOldJobs() {
    ...
    if (oldJobs.length > 0) {
      ...
      a.href = url;
      a.download = `flowjob-archived-jobs-${App.today()}.json`;
      a.click();
