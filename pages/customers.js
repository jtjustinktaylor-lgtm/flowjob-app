...
const Customers = {
  ...
  detail(id) {
    ...
    App.openModal(`
      ...
      ${c.notes ? `<div style="margin-bottom:16px;padding:10px;background:var(--bg);border-radius:8px;font-size:13px"><strong>Notes:</strong> ${App.esc(c.notes)}</div>` : ''}
      ...
      <div class="modal-footer">
        ...
        <button class="btn btn-outline" onclick="App.closeModal();CustomerDetail.newJobFor('${App.esc(c.name)}')">📅 New Job</button>
  ...
  edit(id) {
    ...
    App.openModal(`<div class="modal-header"><h3>Edit Customer</h3><button class="modal-close" onclick="App.closeModal()">✕</button></div>
      <div class="form-group"><label>Name</label><input class="form-control" id="cf-name" value="${App.esc(c.name)}"></div>
      <div class="form-group"><label>Phone</label><input class="form-control" id="cf-phone" type="tel" value="${App.esc(c.phone||'')}"></div>
      <div class="form-group"><label>Email</label><input class="form-control" id="cf-email" type="email" value="${App.esc(c.email||'')}"></div>
      <div class="form-group"><label>Address</label><input class="form-control" id="cf-addr" value="${App.esc(c.address||'')}"></div>
      <div class="form-group"><label>Notes</label><textarea class="form-control" id="cf-notes">${App.esc(c.notes||'')}</textarea></div>
      <div class="modal-footer">
        <button class="btn btn-outline" onclick="App.closeModal()">Cancel</button>
