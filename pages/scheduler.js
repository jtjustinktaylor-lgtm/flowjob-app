      ...
      <div class="card">
        ...
        <div class="table-wrap"><table id="sched-table">
          ...
          <tbody>${(jobs||[]).sort((a,b)=>b.date.localeCompare(a.date)).slice(0,30).map(j => {
            ...
            <td>${App.formatDate(j.date)}</td><td>${App.esc(j.title)}</td><td>${App.esc(j.customer)||'—'}</td>
...
const Scheduler = {
  ...
  edit(id) {
    ...
    App.openModal(`<div class="modal-header"><h3>${j.title?'Edit':'New'} Job</h3><button class="modal-close" onclick="App.closeModal()">✕</button></div>
      <div class="form-group"><label>Title</label><input class="form-control" id="jf-title" value="${App.esc(j.title||'')}"></div>
      <div class="form-group"><label>Customer</label>
        ...
        <input class="form-control" id="jf-cust" value="${App.esc(j.customer||'')}" placeholder="Or type customer name" style="margin-top:6px">
      </div>
      ...
      <div class="form-group"><label>Address</label><input class="form-control" id="jf-addr" value="${App.esc(j.address||'')}"></div>
      ...
      <div class="form-group"><label>Notes</label><textarea class="form-control" id="jf-notes">${App.esc(j.notes||'')}</textarea></div>
